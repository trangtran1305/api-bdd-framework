...
namespace SafeGuardMH.Pages
{
    public class AdditionalDriversPage : BasePage
    {
        ...
        public void ClickAdditionalDriverContinue1(string input)
        {
            if (!String.IsNullOrEmpty(input))
            {
                btnAddAdditionalDriver.Click();
        ...
        public void VerifyErrorMessageDisplayed(string input)
        {
            switch (input)
            {
                ...
                case "UKResidentDate":
                    ...
                    break;
                case "MarritalStatus":
                    Assert.Equal("If an additional driver’s relationship to the proposer is Spouse, then both marital status answers must be Married. Please review the answers in the You - Your Details and Additional Driver - Driver Details sections.", lblErrorMessage.GetText());
                    break;
                ...
                case "MobilePhone":
                    ...
                    break;
                case "AgeLessThan25":
                    Assert.Equal("This driver must be 25 or over for us to quote.", lblErrorMessage.GetText());
                    break;
                case "AgeMoreThan80":
                    Assert.Equal("Please check this driver’s date of birth.", lblErrorMessage.GetText());
                    break;
                default:
        ...
        public void InputDOBLessThan25(string input)
        {
            if (!string.IsNullOrEmpty(input))
            {
                ...
                InputAdditionalDriverBirth(date);
            }
        }
