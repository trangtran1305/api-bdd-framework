...
{
    [Binding]
    public class SavePaymentInfoSGSteps : Steps
    {
        ...
        private string _apiVersionSavePayment, _apiVersionQuote, _contextName;
        ...
        public void GivenUserHasSavePaymentInfoRequest(Table saveDebitTable)
        {
            ...
            _apiVersionSavePayment = paramValues["ApiVersion"];
            _contextName = paramValues["ContextName"];
        }

        [Given(@"Quote Service version ""(.*)"" and Json file is ""(.*)""")]
        public void GivenQuoteServiceVersionAndJsonFileIs(string apiversion, string jsonBody)
        {
            _quoteRequestBody = jsonBody;
            _apiVersionQuote = apiversion;
        }

        [When(@"User send SG Save Payment Info Normal Case")]
        public void WhenUserSendSavePaymentInfoRequest()
        {
            ParametersForRequest lstPara = Common.GetParametersForRequest(_apiVersionSavePayment, _contextName, _savePaymentInfoRequestBody);
            var lstResultObject = new List<ResultObject>();
            lstResultObject = SavePaymentInfoFunctions.GetSessionIdAndTotalPremium(lstPara.Token, _quoteRequestBody, _savePaymentInfoRequestBody, _apiVersionQuote, _contextName);
            Thread.Sleep(2000);
        ...
        public void WhenUserSendSavePaymentInfoRequest(Table table)
        {
            ...
            Thread.Sleep(1000);
            double totalPremiumInQuote = lstResultObject[0].TotalPremium;
            var jsonBody = PrepurchaseFunctions.CreateRequestBody(_savePaymentInfoRequestBody, table, sessionId, _apiVersionSavePayment, _contextName);
        ...
        [Then(@"SG Save Payment Info response returns (.*) and (.*) and (.*)")]
        ...
        [When(@"SG User send Save Payment Info request with (.*)")]
        public void WhenUserSendSavePaymentInfoRequestWith(string input)
        {
            ...
            {
                var lstResultObjectNew = SavePaymentInfoFunctions.GetSessionIdAndTotalPremium(lstPara.Token, _quoteRequestBody, _savePaymentInfoRequestBody, _apiVersionSavePayment, _contextName);
                sessionId = lstResultObject[0].SessionId.ToString();
            }
            double totalPremiumInQuote = lstResultObject[0].TotalPremium;
