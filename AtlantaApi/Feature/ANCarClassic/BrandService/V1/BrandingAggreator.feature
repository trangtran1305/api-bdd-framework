#Refer manual test cases: Atlanta_APi_System Test
#Sheet Branding
#Refer file: Atlanta_API_System Test.xlsx
#Sheet Branding
#TCID: Branding_002
@ANCarClassic
Feature: Get Branding with aggreator

@BrandingServices
Scenario Outline: Get Branding with aggregator
	Given User has Reference Branding Data body
	 | Property    | Value        |
	 | ApiVersion  | V1           |
	 | ContextName | ANCarClassic |
	When User sends get branding request using <AppUrl>
	Then Branding response should returns <StatusCode> and <IsSuccess> and <Messages>
	Examples:
	| AppUrl                      |  StatusCode | IsSuccess | Messages                                |
	| /api/v1/branding/aggregator?source=CTMT |  200        | true      | Get Aggregator information successfully |