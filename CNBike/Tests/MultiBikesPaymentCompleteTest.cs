...
{
    public class MultiBikesPaymentCompleteTest 
    {      
        static string _dataFile = Directory.GetCurrentDirectory() + "\\TestData\\RegressiontestData.xlsx";
        static string _sheetName = "MultiBikesPaymentComplete"; 
        static string _expectedResult = "Passed";

        [Theory]
        [MemberData(nameof(GetMultiBikeWithABICodeData))]

        public void TestMultiBikesComplete(string testCaseID, string testCaseDesc, string testResult)
        {
